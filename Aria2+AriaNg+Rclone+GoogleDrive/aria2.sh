...
Aria2_RPC_API="http://127.0.0.1:6800/jsonrpc"
...
Update_bt_tracker_cron(){
	check_installed_status
	# 检查 Aria2 中是否有任务，如果没有才允许更新 BT-Tracker 后重启 Aria2，否则在有任务时重启 Aria2 的时候会造成任务状态异常
	Read_config
	global_stat=$(wget -qO- --post-data="{\"jsonrpc\":\"2.0\",\"method\":\"aria2.getGlobalStat\",\"id\":\"Update_bt_tracker_cron\",\"params\":[\"token:${aria2_passwd}\"]}" $Aria2_RPC_API)
	numActive=$(parse_json $global_stat "numActive")
	numWaiting=$(parse_json $global_stat "numWaiting")
	numStopped=$(parse_json $global_stat "numStopped")
